...
import com.masum.gallery.model.DefaultResponse
...
class GalleryApiUseCase @Inject constructor(private val repository: GalleryRepository) {
    ...
    operator fun invoke(page: String): Flow<ResponseResult<GalleryResponse>> = flow {
        try {
            emit(ResponseResult.Loading())
            val appResponse = repository.loadGallery(page)
            emit(ResponseResult.Success(appResponse))
        } catch (e: HttpException) {
            val error = ResponseResult.Error<GalleryResponse>(e.localizedMessage ?: "An unexpected error")
            emit(error)
        } catch (e: IOException) {
            val error =
                ResponseResult.Error<GalleryResponse>("Couldn't reach server. Check your internet connection.")
            emit(error)
        }
    }
